...
function buildFilterUrl($params = []) {
    ...
    return 'products.php?' . http_build_query($new);
}
    ...
    <style>
        ...
        * {
        ...
        .color-preview {
            ...
            flex-shrink: 0; /* Prevent color dot from shrinking */
        }

        .price-inputs { /* This class is no longer used but kept for context if needed */
            display: grid;
            grid-template-columns: 1fr 1fr;
            gap: 1rem;
        }

        .price-input { /* This class is no longer used but kept for context if needed */
            width: 100%;
            padding: 0.75rem;
            border: 1px solid #d2d2d7;
            border-radius: 8px;
        ...
        .price-input:focus { /* This class is no longer used but kept for context if needed */
            outline: none;
            border-color: #0066cc;
        }
        ...
        .active-filters-container {
            ...
            box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);
            margin-bottom: 2rem;
            display: none; /* Hidden by default, shown by JS if filters active */
        }
        ...
        color-options { /* This class name seems incorrect, likely meant to be .product-colors */
            display: flex;
            flex-wrap: wrap;
            gap: 0.5rem;
            margin-bottom: 1rem;
...
<body>
    ...
    <div class="loading"></div> <main class="products-container">
        <aside class="sidebar">
            <form action="" method="GET" id="filter-form">
                ...
                <div class="filter-section">
                    ...
                    <div class="filter-options">
                        <a href="<?= buildFilterUrl(['category_id' => 0]) ?>" 
                           class="<?= $category_id == 0 ? 'active' : '' ?>">
                            Tất cả sản phẩm
                        </a>
                        <?php foreach($category_map as $id => $name): ?>
                            <a href="<?= buildFilterUrl(['category_id' => $id]) ?>"
                    ...
                    <div class="checkbox-group">
                        <?php foreach($color_options as $color_name => $hex): ?>
                            <label class="checkbox-label">
                                <input type="checkbox" 
                                       name="color[]" 
                                       value="<?= htmlspecialchars($color_name) ?>"
        ...
        <div class="main-content">
            <div class="active-filters-container">
                ...
                <?php /* The clear all button is now managed by JavaScript */ ?>
                    <a href="products.php" class="clear-all-filters">Xóa tất cả bộ lọc</a>
            </div>
    ...
    <script>
            ...
            if (filters.length > 0) {
                ...
                activeFiltersContainer.querySelectorAll('.remove-filter').forEach(removeBtn => {
                    removeBtn.addEventListener('click', function() {
                        ...
                        window.location.href = 'products.php?' + currentParams.toString();
                    });
                });
        ...
        document.querySelectorAll('a').forEach(link => {
            if (link.href.includes('products.php')) {
